...
export async function login (loginInfo : ILoginInput) {
    ...
    return await fetchDataRetry(async () => await errorHandlingWrapper(`${baseAPIPath}/user/login`, loginInfo, "post"), 0, recallingAPILimit);
}

export async function register (registerInfo : IRegisterInput) {
    console.log("Trying to register");
    return await fetchDataRetry(async () => await errorHandlingWrapper(`${baseAPIPath}/user/register`, registerInfo, "post"), 0, recallingAPILimit);
}

export async function getSavedGame () {
    console.log("Trying to get saved game");
    return await fetchDataRetry(async () => await errorHandlingWrapper(`${baseAPIPath}/gameData/mine`, {}, "get"), 0, recallingAPILimit);
}